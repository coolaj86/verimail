...
(function () {
  ...
  function Verimail(opts) {
    ...
    setTimeout(function () {
      ...
      me._db.keys().forEach(function (id) {
        ...
        if (stat.verifiedAt && !stat.unsubscribedAt && !stat.handledAt) {
  ...
  Verimail.prototype.query = function (email, cb) {
    ...
    me.cipher(email, function (err, id) {
      ...
      // TODO some way to overwrite these error messages
      if (stat.verifiedAt && !stat.unsubscribedAt) {
        cb(new Error('This address already is verified'), stat);
        return;
      }

      if (stat.sentConfirmationAt && !stat.unsubscribedAt) {
